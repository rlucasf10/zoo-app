...
class AdminController
{
    ...
    public function getAllItineraries($adminId)
    {
        ...
        global $pdo;
        $stmt = $pdo->prepare("SELECT * FROM itinerarios");
        $stmt->execute();
    ...
    public function deleteItinerary($itineraryId, $adminId)
    {
        ...
        $stmt = $pdo->prepare("SELECT id FROM itinerarios WHERE id = :id");
        $stmt->bindParam(':id', $itineraryId);
        $stmt->execute();

        if ($stmt->rowCount() > 0) {
            // Eliminar el itinerario
            $stmt = $pdo->prepare("DELETE FROM itinerarios WHERE id = :id");
            $stmt->bindParam(':id', $itineraryId);
    ...
    public function getAllSpecies($adminId)
    {
        ...
        global $pdo;
        $stmt = $pdo->prepare("SELECT * FROM especies");
        $stmt->execute();
    ...
    public function deleteSpecies($speciesId, $adminId)
    {
        ...
        $stmt = $pdo->prepare("SELECT id FROM especies WHERE id = :id");
        $stmt->bindParam(':id', $speciesId);
        $stmt->execute();

        if ($stmt->rowCount() > 0) {
            // Eliminar la especie
            $stmt = $pdo->prepare("DELETE FROM especies WHERE id = :id");
            $stmt->bindParam(':id', $speciesId);
